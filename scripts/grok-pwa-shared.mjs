...
export function grokPwaHeadTags(appName = DEFAULT_APP_NAME) {
  return [
    ...
    ["manifest", '<link rel="manifest" href="/helper/manifest.webmanifest">'],
    ["apple-touch-icon", '<link rel="apple-touch-icon" href="/helper/icon-180.png">'],
    [
...
export function injectGrokPwaHead(html, ctx = {}) {
  ...
  const missing = grokPwaHeadTags(appName)
    .filter(([key]) => {
      if (key === "manifest") return !next.includes('href="/helper/manifest.webmanifest"');
      if (key === "apple-touch-icon") return !next.includes('href="/helper/icon-180.png"');
