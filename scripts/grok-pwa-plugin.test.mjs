...
test("filters non-document paths", () => {
  ...
  assert.equal(isDocumentPath("/helper/install/styles.css"), false);
...
test("renders install page markup", () => {
  ...
  assert.match(html, /\/helper\/install\/styles\.css/);
...
test("renders the manifest with the per-app name", () => {
  ...
  assert.equal(manifest.icons[0].src, "/helper/icon-180.png");
});
...
test("nitro middleware and its bundled assets exist", () => {
  ...
  readFileSync(join(TEMPLATE_ROOT, "public/helper/icon-180.png"));
  readFileSync(join(TEMPLATE_ROOT, "public/helper/install/styles.css"));
});
